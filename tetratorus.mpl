...
HasSelfIntersections := proc(surface,coor) 
  local f, e, v, v1, v2, w, w1, mat, sol, matin, c, temp,sol1;
  c:=coor; #converting coor to radical might improve the algorithm
  #print("starting selfint");
  for f in Faces(surface) do  
    for e in select(e -> not (e[1] in f or e[2] in f), Edges(surface)) do 
      v := c[f[1]]; 
      v1 := c[f[2]] - c[f[1]]; 
      v2 := c[f[3]] - c[f[1]]; Print("f=",f,"e=",e);
      w := c[e[1]]; 
      w1 := c[e[2]] - c[e[1]]; 
      mat := simplify(<<v1> | <v2> | <-w1>>); #print(evala(Determinant(mat)));
      if is(simplify(Determinant(mat))<>0) then #not(verify(Determinant(mat),0)) then  #is(Determinant(mat)<>0)
        matin := MatrixInverse(mat); 
        sol := simplify(matin . <w - v>);   #sol := matin . <w - v>; #,evalf(sol),is(numer(sol[1])<denom(sol[1])));
        #sol1:=(sol[1]+sol[2]);
        #if verify(sol[1],0..1,'truefalse([interval])') and verify(sol[2],0..1,'truefalse([interval])') and verify(sol[3],0..1,'truefalse([interval])') then # and verify(sol1,[-infinity,1],'truefalse([interval])') then
        #print("here?");
        if (is(sol[3]>=0) and is(sol[3]<=1)) and is(sol[2]>=0) and is(sol[1]>=0) and is(sol[1] + sol[2] <= 1) then 
          #print("here1",evalf(sol));
          return true;
        end if;
      end if; 
    end do; 
      ...
      if nops({op(face1)} intersect {op(face2)})=0 and not(HasEdgeInPlane(surface, face1, face2)) then 
        ...
        for sol in solution do #print("check1");
          ...
          if type(evalf(aa),float) and type(evalf(bb),float) and aa<>0 and bb<> 0 then 
            ...
            print("boundary s=",Boundary(s)); 
            data_identify:=HasIdentifiedFaces(s,face1,face2);
            if data_identify[1] then
              f1:=data_identify[2];
              f2:=data_identify[3]; 
              ss:=IdentifyFaces(s,f1,f2); ##check this function
            ...
            print("boundary ss=",Boundary(ss));
             print(check2);
            if IsVertexfaithful(coord) and is(EulerCharacteric(ss)=0) then 
              bool_selfint:=HasSelfIntersections(ss,coord);  #print(HasSelfIntersections(ss,coord));
              #bool_selfint:=false;
              if check_mirror then 
                bool_mirror:=HasMirrorSymmetries(ss); #print(HasMirrorSymmetries(ss));
                #bool_mirror:=false;
              else 
